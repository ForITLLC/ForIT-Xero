...
/**
 * Distinct customer_ids that currently hold a Xero connection row.
 * Used by the keep-alive timer to refresh every live connection on a
 * schedule so no refresh token idles past Xero's 60-day TTL.
 */
export async function getConnectedCustomerIds(): Promise<string[]> {
  const db = await getDbPool();
  const result = await db.request()
    .query('SELECT DISTINCT customer_id FROM xero.xero_connections');
